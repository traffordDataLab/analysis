...
cc_trafford <- read_csv("https://www.nomisweb.co.uk/api/v01/dataset/NM_162_1.data.csv?geography=1811939363&date=latestMINUS69-latest&gender=0&age=0&measure=1,2&measures=20100") %>%
...
latestch <- "October 2025"
latestm12ch <- "October 2024"
...
ggplot(cc_traffordp,aes(x = date, y = round(change,0))) + 
  ...
  scale_x_date(breaks = seq(as.Date("2020-02-01"), as.Date(max(cc_traffordp$period)), by="4 months"), date_labels="%b %y", expand = c(0.03,0.03)) +
...
The Trafford MSOA with the largest share of claims in `r latestch` was `r filter(c_count, Share == max(Share)) %>% pull(area_name)` with `r filter(c_count, Share == max(Share))$Share`% of all claimants within Trafford, almost the double of claims than Gorse Hill, the MSOA with the second largest number of claims. 4 of the MSOA has more claims in `r latestch`  compared to `r latestm12ch`, and 8 MSOA had no additional claims. `r arrange(c_count, desc(additional_claims))$area_name[1]` have the largest increase in number of claims with `r abs(arrange(c_count, desc(additional_claims))$additional_claims[1])` more, followed by `r arrange(c_count, desc(additional_claims))$area_name[2]`, `r arrange(c_count, desc(additional_claims))$area_name[3]` and `r arrange(c_count, desc(additional_claims))$area_name[4]` with `r arrange(c_count, desc(additional_claims))$additional_claims[2]` additional claims each. `r arrange(c_count, additional_claims)$area_name[1]` had the most reduction in number of claims with `r abs(arrange(c_count, additional_claims)$additional_claims[1])` less followed by `r arrange(c_count, additional_claims)$area_name[2]` (`r abs(arrange(c_count, additional_claims)$additional_claims[2])`).
...
tbl2 <- reactable(
  ...
  columns = list(
    ...
    Share = colDef(name = "Share Oct 25", maxWidth = 80, align = "left"),
...
raw_cc_age <- read_csv("https://www.nomisweb.co.uk/api/v01/dataset/NM_162_1.data.csv?geography=1811939363&date=latestMINUS69-latest&gender=0&age=0,2,10,11,3,12...16,4,17...20&measure=1&measures=20100") 
...
In Trafford, from  `r latestm12ch` to `r latestch`, 8 out of 11 of the age bands has decreased the number of claimants. The age band `r filter(table3, additional_claims == max(additional_claims)) %>% pull(Age)`  had the most increase with `r filter(table3, additional_claims == max(additional_claims)) %>% pull(additional_claims)` more claimants. When considering the reduction of claims the age band `r filter(table3, additional_claims == min(additional_claims)) %>% pull(Age)` years had the largest reduction with `r filter(table3, additional_claims == min(additional_claims)) %>% pull(additional_claims)` less claims. Considering the broad age bands, only the age band 16-24 had increases.
...
ggplot(uc_plot_ch,aes(x = date, y = change)) + 
  ...
  geom_point(size = 2, alpha = 1, color="#006d2c") + 
    geom_text(data=uc_plot_ch %>% filter(!period==base),
            ...
            fontface = 1.5, size = 2.5, hjust = 0.3, vjust = -1.3)+
  scale_color_manual(values = c("#006d2c", "transparent")) +
  scale_x_date(breaks = seq(as.Date("2020-02-01"), as.Date(max(uc_plot_ch$period)), by="4 months"), date_labels="%b %y", expand = c(0.03,0.03)) +
...
query <- list(database = unbox("str:database:UC_Monthly"),
              ...
              recodes = list(
                ...
                `str:field:UC_Monthly:F_UC_DATE:DATE_NAME` = list(
                  map = as.list(paste0("str:value:UC_Monthly:F_UC_DATE:DATE_NAME:C_UC_DATE:",c(202410,202510))))
              )) %>% toJSON()
request <- POST(
  url = path,
  body = query,
...
query <- list(database = unbox("str:database:UC_Monthly"),
              ...
              recodes = list(
                `str:field:UC_Monthly:V_F_UC_CASELOAD_FULL:COA_CODE` = list(
                  map = as.list(paste0("str:value:UC_Monthly:V_F_UC_CASELOAD_FULL:COA_CODE:V_C_MASTERGEOG21_MSOA_TO_LA:E0", seq(2001259, 2001286, 1)))),
                `str:field:UC_Monthly:F_UC_DATE:DATE_NAME` = list(
                  map = as.list(paste0("str:value:UC_Monthly:F_UC_DATE:DATE_NAME:C_UC_DATE:",c(202510)))),
...
uc_plot_entitlement_c <- uc_entitlement_trend %>%
 ...
 ggplot(uc_plot_entitlement_c, aes(x = date, y = value, colour = indicator)) +
  ...
  scale_x_date(breaks = c(min(uc_plot_entitlement_c$date), as.Date(as.yearmon("Jan 2021"),format="%b %Y"), as.Date(as.yearmon("Jan 2022"),format="%b %Y"),  as.Date(as.yearmon("Jan 2023"),format="%b %Y"), as.Date(as.yearmon("Jan 2024"),format="%b %Y"), as.Date(as.yearmon("Jan 2025"),format="%b %Y")), date_labels = "%b %y") +
  scale_y_continuous(expand = c(0.005, 0.005)) +
