...
library(zoo) ;library(sf) ; library(jsonlite) ; library(scales) ; library(kableExtra)
...
cc_trafford <- read_csv("https://www.nomisweb.co.uk/api/v01/dataset/NM_162_1.data.csv?geography=1811939363&date=latestMINUS13-latest&gender=0&age=0&measure=1,2&measures=20100") %>%
...
The [Claimant Count](https://www.nomisweb.co.uk/sources/cc) indicates the number of people claiming benefits principally for the reason of being unemployed. From `r base` to `r latest` the Claimant Count in Trafford has increased a `r filter(cc_traf,period==as.yearmon(latest, "%b %Y"))$change`%.
The rate of claimants to residents aged 16 to 64 reached `r filter(cc_traf,period==as.yearmon(latest, "%b %Y"))$rate`% in `r latest` representing an increment of `r filter(cc_traf,period==as.yearmon(latest, "%b %Y"))$rate-cc_traf$rate[1]`% from `r base`.
...
tbl1 <- reactable(
  ...
  fullWidth = FALSE,
  defaultColDef = colDef(headerClass = "header", align = "right"),
  columns = list(
...
ggplot(cc_traffordp,aes(x = date, y = round(change,0))) + 
  ...
  geom_point(size = 5, alpha = 0.8, color="#66c2a4") + 
    geom_text(data=cc_traffordp %>% filter(!period==base),
            aes(  
                label = paste0(round(change,0), "%")), 
            color = "#66c2a4", size = 4, hjust = 0, vjust = 2) +
...
The Trafford ward with the largest share of claims in `r latest` was `r filter(c_count, Share == max(Share)) %>% pull(area_name)` with `r filter(c_count, Share == max(Share))$Share`% of all claimants within Trafford. The ward with the highest percentage change from `r latestm12` to `r latest` is `r filter(c_count, percentage_change == max(percentage_change)) %>% pull(area_name)` with `r filter(c_count, percentage_change == max(percentage_change)) %>% pull(percentage_change)`% change. However, the ward with more additional claims in this period is `r filter(c_count, additional_claims == max(additional_claims)) %>% pull(area_name)` with `r filter(c_count, additional_claims == max(additional_claims)) %>% pull(additional_claims)`, followed by `r arrange(c_count, desc(additional_claims))$area_name[2]` (`r arrange(c_count, desc(additional_claims))$additional_claims[2]`), `r arrange(c_count, desc(additional_claims))$area_name[3]`(`r arrange(c_count, desc(additional_claims))$additional_claims[3]`), `r arrange(c_count, desc(additional_claims))$area_name[4]`(`r arrange(c_count, desc(additional_claims))$additional_claims[4]`) and `r arrange(c_count, desc(additional_claims))$area_name[5]`(`r arrange(c_count, desc(additional_claims))$additional_claims[5]`).
...
tbl2 <- reactable(
  ...
  columns = list(
    ...
    additional_claims = colDef(
      ...
      cell = function(value) {
        width <- paste0(value * 100 / max(table2$additional_claims), "%")
        value <- format(value, big.mark = ",")
        value <- format(value, width = 9, justify = "right")
        bar <- div(
          class = "bar-chart",
          style = list(marginRight = "6px"),
          div(class = "bar", style = list(width = width, backgroundColor = "#66c2a4"))
        )
        div(class = "bar-cell", span(class = "number", value), bar)
      }
    ),
    Share = colDef(name = "Share Feb 21", maxWidth = 80, align = "left"),
    ...
    `Change` = colDef(
      cell = function(value) {
        width <- paste0(value * 100 / max(table2$`Change`), "%")
        value <- format(value, big.mark = ",")
        value <- format(value, width = 9, justify = "right")
        bar <- div(
          class = "bar-chart",
...
The wards with higher claimant rate in `r latest` were `r filter(table2, latest_r == max(latest_r)) %>% pull(area_name)` with `r filter(table2, latest_r == max(latest_r))$latest_r`% and `r arrange(table2, desc(latest_r))$area_name[2]` with `r arrange(table2, desc(latest_r))$latest_r[2]`%. The wards where the claimant rate has grown more from `r latestm12` to `r latest` are also `r filter(table2, Change == max(Change)) %>% pull(area_name)` and `r arrange(table2, desc(Change))$area_name[3]` with an increase in rate of `r arrange(table2, desc(Change))$Change[1]`% and `r arrange(table2, desc(Change))$Change[3]`% respectively along with `r arrange(table2, desc(Change))$area_name[2]` with `r arrange(table2, desc(Change))$Change[2]`%.
...
c_rate_ward <- st_read("https://www.traffordDataLab.io/spatial_data/ward/2017/trafford_ward_generalised.geojson", quiet = TRUE) %>% 
...
raw_cc_age <- read_csv("https://www.nomisweb.co.uk/api/v01/dataset/NM_162_1.data.csv?geography=1811939363&date=latestMINUS13-latest&gender=0&age=0,2,10,11,3,12...16,4,17...20&measure=1&measures=20100") 
...
In Trafford from  `r latestm12` to `r latest` almost all age bands have at least doubled their claimant count. When considering the number of additional claims the age band `r filter(table3, latest == max(latest)) %>% pull(Age)` years had the highest with `r filter(table3, latest == max(latest)) %>% pull(latest)` more claims.
...
tbl3 <- reactable(
  ...
  columns = list(
    ...
    additional_claims = colDef(
      name = "Additional claims",
      cell = function(value) {
        width <- paste0(value * 100 / max(table3$additional_claims), "%")
        value <- format(value, big.mark = ",")
        value <- format(value, width = 9, justify = "right")
        bar <- div(
          class = "bar-chart",
    ...
    percentage_change = colDef(
      name = "% Change",
      cell = function(value) {
        width <- paste0(value * 100 / max(table3$percentage_change), "%")
        value <- format(value, big.mark = ",")
        value <- format(value, width = 9, justify = "right")
        bar <- div(
          class = "bar-chart",
...
tbl4 <- reactable(
  ...
  columns = list(
    ...
    percentage_change = colDef(
      name = "% Change",
      cell = function(value) {
        width <- paste0(value * 100 / max(table4$percentage_change), "%")
        value <- format(value, big.mark = ",")
        value <- format(value, width = 9, justify = "right")
        bar <- div(
          class = "bar-chart",
...
At the start of 2020 the age band 25-34 already had the highest number of claims however after the effects of the Covid-19 pandemic that started in Mar 2020 this figure increased more compared to the other age bands. The claimant rate as a proportion of the Traford's population of each age band shows a higher increase in the age bands of 25-34 and 16-24 compared to the others.
...
plotdf <- quinary %>%
 ...
 ggplot(plotdf, aes(x = date, y = value, colour = Age)) +
  ...
  geom_text(aes(x = max(date), label = label, fontface = "bold", hjust = 0), show.legend = FALSE) +
  scale_x_date(breaks = c(min(plotdf$date), as.Date(as.yearmon("Mar 2020"),format="%b %Y"), max(plotdf$date)), date_labels = "%b %Y") +
  scale_y_continuous(expand = c(0.005, 0.005)) +
  labs(title = "Claimant Count by age band",
       subtitle = paste("Trafford, ", latestm12, " - ",latest),
       caption = "Source: ONS",
       x = NULL,
       y = NULL,
       colour = NULL) +
...
plotdf2 <- quinary %>%
  ...
  mutate(label = if_else(date == "2021-02-01", Age, factor(NA)))
...
Almost half of Trafford's claimants are residents of five of the 21 Trafford's wards. Almost half of the claimants are under 34 years.
...
tbl5 <- reactable(
  ...
  defaultSorted = "Ward",
  defaultSortOrder = "asc",
  defaultColDef = colDef(headerClass = "header", align = "left", maxWidth = 45),
  columns = list(
...
query <- list(database = unbox("str:database:UC_Monthly"),
              ...
              recodes = list(
                ...
                `str:field:UC_Monthly:F_UC_DATE:DATE_NAME` = list(
                  map = as.list(paste0("str:value:UC_Monthly:F_UC_DATE:DATE_NAME:C_UC_DATE:",c(202001,202002,202003,202004,202005,202006,202007,202008,202009,202010,202011,202012,202101,202102))))
              )) %>% toJSON()
request <- POST(
  url = path,
  body = query,
...
query <- list(database = unbox("str:database:UC_Monthly"),
              ...
              recodes = list(
                ...
                `str:field:UC_Monthly:F_UC_DATE:DATE_NAME` = list(
                  map = as.list(paste0("str:value:UC_Monthly:F_UC_DATE:DATE_NAME:C_UC_DATE:",c(202002,202102))))
              )) %>% toJSON()
request <- POST(
  url = path,
  body = query,
...
The Trafford MSOA with the largest share of claims in `r latest` was `r filter(table7, Share == max(Share)) %>% pull(area_name)` with `r filter(table7, Share == max(Share))$Share`% of all Universal Credit claims in Trafford. The MSOA with the highest percentage change from `r latestm12` to `r latest` is `r filter(table7, percentage_change == max(percentage_change)) %>% pull(area_name)` with `r filter(table7, percentage_change == max(percentage_change)) %>% pull(percentage_change)`% change. However, the MSOA with more additional claims in this period is `r filter(table7, additional_claims == max(additional_claims)) %>% pull(area_name)` with `r filter(table7, additional_claims == max(additional_claims)) %>% pull(additional_claims)`, followed by `r arrange(table7, desc(additional_claims))$area_name[2]` (`r arrange(table7, desc(additional_claims))$additional_claims[2]`), `r arrange(table7, desc(additional_claims))$area_name[3]`(`r arrange(table7, desc(additional_claims))$additional_claims[3]`), `r arrange(table7, desc(additional_claims))$area_name[4]`(`r arrange(table7, desc(additional_claims))$additional_claims[4]`) and `r arrange(table7, desc(additional_claims))$area_name[5]`(`r arrange(table7, desc(additional_claims))$additional_claims[5]`).
...
tbl7 <- reactable(
  ...
  defaultSorted = "area_name",
  defaultSortOrder = "asc",
  defaultColDef = colDef(headerClass = "header", align = "right", maxWidth = 60),
  columns = list(
    ...
    additional_claims = colDef(
      ...
      align = "left",
      cell = function(value) {
        width <- paste0(value * 100 / max(table7$additional_claims), "%")
        value <- format(value, big.mark = ",")
    ...
    `Change` = colDef(
      maxWidth = 175,
      cell = function(value) {
        width <- paste0(value * 100 / max(table7$`Change`), "%")
        value <- format(value, big.mark = ",")
        value <- format(value, width = 5, justify = "right")
        bar <- div(
          class = "bar-chart",
...
div(class = "div",
div(class= "large-table tableTitle", "Change on the Universal Credit claims for Trafford's MSOAs"),
tbl7
)

```

The MSOAs with higher rates of Universal Credit claims in `r latest` were `r filter(table7, latest_rate == max(latest_rate)) %>% pull(area_name)` with `r filter(table7, latest_rate == max(latest_rate))$latest_rate`% and `r arrange(table7, desc(latest_rate))$area_name[2]` with `r arrange(table7, desc(latest_rate))$latest_rate[2]`%. The MSOAs where the rate of Universal Credit claims has grown more from `r latestm12` to `r latest` were also `r filter(table7, Change == max(Change)) %>% pull(area_name)` and `r arrange(table7, desc(Change))$area_name[2]` with `r arrange(table7, desc(Change))$Change[1]`% and `r arrange(table7, desc(Change))$Change[2]`% respectively along with `r arrange(table7, desc(Change))$area_name[3]` with `r arrange(table7, desc(Change))$Change[3]`%.
...
In Trafford from  `r latestm12` to `r latest` almost all 5-year age bands have increased at least an 80% in the number of Universal Credit claims. When considering the number of additional claims the 5-year age band `r filter(table8, latest == max(latest)) %>% pull(Age)` years had the highest with `r filter(table8, latest == max(latest)) %>% pull(latest)` more claims.
...
tbl8 <- reactable(
  ...
  columns = list(
    ...
    additional_claims = colDef(
      ...
      maxWidth = 175,
      cell = function(value) {
        width <- paste0(value * 100 / max(table8$additional_claims), "%")
        value <- format(value, big.mark = ",")
        value <- format(value, width = 9, justify = "right")
        bar <- div(
          class = "bar-chart",
    ...
    percentage_change = colDef(
      ...
      maxWidth = 175,
      cell = function(value) {
        width <- paste0(value * 100 / max(table8$percentage_change), "%")
        value <- format(value, big.mark = ",")
        value <- format(value, width = 9, justify = "right")
        bar <- div(
          class = "bar-chart",
...
div(class = "div",
div(class= "large-table tableTitle", "Change in the Universal Credit claims by age nabd for Trafford"),
tbl8
)
...
Following the start of the Covid-19 pandemic in Mar 2020 the number of claims for all ages suddenly rose from the pre-pandemic levels at a similar proportion for all age bands with the age band 16-24 rising faster in May 2020. All age bands claims are still slowly rising. At the start of 2020 the age band 25-34 had the highest number of claims and also the highest rate maintaining that position after the rise from Mar 2020.
...
uc_plot_age_c <- traf_uc %>%
 ...
 ggplot(uc_plot_age_c, aes(x = date, y = value, colour = Age)) +
  ...
  geom_text(aes(x = max(date), label = label, fontface = "bold", hjust = 0), show.legend = FALSE) +
  #scale_color_startrek() +
  scale_x_date(breaks = c(min(uc_plot_age_c$date), as.Date(as.yearmon("Mar 2020"),format="%b %Y"), max(uc_plot_age_c$date)), date_labels = "%b %Y") +
  scale_y_continuous(expand = c(0.005, 0.005)) +
...
uc_plot_age_r <- traf_uc %>%
 ...
 ggplot(uc_plot_age_r, aes(x = date, y = rate, colour = Age)) +
  ...
  geom_text(aes(x = max(date), label = label, fontface = "bold", hjust = 0), show.legend = FALSE) +
  #scale_color_startrek() +
  scale_x_date(breaks = c(min(uc_plot_age_r$date), as.Date(as.yearmon("Mar 2020"),format="%b %Y"), max(uc_plot_age_r$date)), date_labels = "%b %Y") +
  scale_y_continuous(expand = c(0.005, 0.005)) +
...
query <- list(database = unbox("str:database:UC_Monthly"),
              ...
              recodes = list(
                `str:field:UC_Monthly:V_F_UC_CASELOAD_FULL:COA_CODE` = list(
                  map = as.list(paste0("str:value:UC_Monthly:V_F_UC_CASELOAD_FULL:COA_CODE:V_C_MASTERGEOG11_MSOA_TO_LA:E0", seq(2001259, 2001286, 1)))),
                `str:field:UC_Monthly:F_UC_DATE:DATE_NAME` = list(
                  map = as.list(paste0("str:value:UC_Monthly:F_UC_DATE:DATE_NAME:C_UC_DATE:",c(202102)))),
