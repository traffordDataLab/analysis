...
cc_trafford <- read_csv("https://www.nomisweb.co.uk/api/v01/dataset/NM_162_1.data.csv?geography=1811939363&date=latestMINUS14-latest&gender=0&age=0&measure=1,2&measures=20100") %>%
...
The [Claimant Count](https://www.nomisweb.co.uk/sources/cc) indicates the number of people claiming benefits principally for the reason of being unemployed. From `r basech` to `r latestch` the Claimant Count in Trafford has increased a `r filter(cc_traf,period==as.yearmon(latest, "%b %Y"))$change`%.
The rate of claimants to residents aged 16 to 64 reached `r filter(cc_traf,period==as.yearmon(latest, "%b %Y"))$rate`% in `r latestch` representing an increment of `r filter(cc_traf,period==as.yearmon(latest, "%b %Y"))$rate-cc_traf$rate[1]`% from `r basech`.
...
tbl2 <- reactable(
  ...
  columns = list(
    ...
    Share = colDef(name = "Share Mar 21", maxWidth = 80, align = "left"),
...
The wards with higher claimant rate in `r latestch` were `r filter(table2, latest_r == max(latest_r)) %>% pull(area_name)` with `r filter(table2, latest_r == max(latest_r))$latest_r`% and `r arrange(table2, desc(latest_r))$area_name[2]` with `r arrange(table2, desc(latest_r))$latest_r[2]`%. The wards where the claimant rate has grown more from `r latestm12ch` to `r latestch` are also `r filter(table2, Change == max(Change)) %>% pull(area_name)` and `r arrange(table2, desc(Change))$area_name[3]` with an increase in rate of `r arrange(table2, desc(Change))$Change[1]`% and `r arrange(table2, desc(Change))$Change[3]`% respectively along with `r arrange(table2, desc(Change))$area_name[2]` with `r arrange(table2, desc(Change))$Change[2]`%.
...
raw_cc_age <- read_csv("https://www.nomisweb.co.uk/api/v01/dataset/NM_162_1.data.csv?geography=1811939363&date=latestMINUS14-latest&gender=0&age=0,2,10,11,3,12...16,4,17...20&measure=1&measures=20100") 
...
In Trafford from  `r latestm12ch` to `r latestch` almost all age bands have at least doubled their claimant count. When considering the number of additional claims the age band `r filter(table3, additional_claims == max(additional_claims)) %>% pull(Age)` years had the highest with `r filter(table3, additional_claims == max(additional_claims)) %>% pull(additional_claims)` more claims.
...
tbl3 <- reactable(
  ...
  columns = list(
    ...
    percentage_change = colDef(
      name = "% Change",
      cell = function(value) {
        width <- paste0(value * 100 / max(table3$percentage_change, na.rm = TRUE),"%")
        value <- format(value, big.mark = ",")
        value <- format(value, width = 9, justify = "right")
        bar <- div(
          class = "bar-chart",
...
plotdf2 <- quinary %>%
  ...
  mutate(label = if_else(date == "2021-03-01", Age, factor(NA)))
...
query <- list(database = unbox("str:database:UC_Monthly"),
              ...
              recodes = list(
                ...
                `str:field:UC_Monthly:F_UC_DATE:DATE_NAME` = list(
                  map = as.list(paste0("str:value:UC_Monthly:F_UC_DATE:DATE_NAME:C_UC_DATE:",c(202001,202002,202003,202004,202005,202006,202007,202008,202009,202010,202011,202012,202101,202102,202103))))
              )) %>% toJSON()
request <- POST(
  url = path,
  body = query,
...
query <- list(database = unbox("str:database:UC_Monthly"),
              ...
              recodes = list(
                ...
                `str:field:UC_Monthly:F_UC_DATE:DATE_NAME` = list(
                  map = as.list(paste0("str:value:UC_Monthly:F_UC_DATE:DATE_NAME:C_UC_DATE:",c(202003,202103))))
              )) %>% toJSON()
request <- POST(
  url = path,
  body = query,
...
In Trafford from  `r latestm12ch` to `r latestch` almost all 5-year age bands have increased at least an 80% in the number of Universal Credit claims. When considering the number of additional claims the 5-year age band `r filter(table8, additional_claims == max(additional_claims)) %>% pull(Age)` years had the highest with `r filter(table8, additional_claims == max(additional_claims)) %>% pull(additional_claims)` more claims.
...
query <- list(database = unbox("str:database:UC_Monthly"),
              ...
              recodes = list(
                `str:field:UC_Monthly:V_F_UC_CASELOAD_FULL:COA_CODE` = list(
                  map = as.list(paste0("str:value:UC_Monthly:V_F_UC_CASELOAD_FULL:COA_CODE:V_C_MASTERGEOG11_MSOA_TO_LA:E0", seq(2001259, 2001286, 1)))),
                `str:field:UC_Monthly:F_UC_DATE:DATE_NAME` = list(
                  map = as.list(paste0("str:value:UC_Monthly:F_UC_DATE:DATE_NAME:C_UC_DATE:",c(202103)))),
