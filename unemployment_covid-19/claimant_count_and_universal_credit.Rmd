...
cc_trafford <- read_csv("https://www.nomisweb.co.uk/api/v01/dataset/NM_162_1.data.csv?geography=1811939363&date=latestMINUS20-latest&gender=0&age=0&measure=1,2&measures=20100") %>%
...
latestch <- "September 2021"
latestm12ch <- "September 2020"
...
The rate of claimants to residents aged 16 to 64 reached `r filter(cc_traf,period==as.yearmon(latest, "%b %Y"))$rate`% in `r latestch` representing an increment of `r filter(cc_traf,period==as.yearmon(latest, "%b %Y"))$rate-cc_traf$rate[1]`% from `r basech`. The number of claimants has been decreased from April 2021. 
...
ggplot(cc_traffordp,aes(x = date, y = round(change,0))) + 
  ...
  geom_point(size = 5, alpha = 0.8, color="#66c2a4") + 
    geom_text(data=cc_traffordp %>% filter(!period==base),
            ...
            color = "#66c2a4", size = 3, hjust = 0, vjust = 2) +
  scale_x_date(breaks = cc_traffordp$date, date_labels="%b %y") +
...
tbl2 <- reactable(
  ...
  columns = list(
    ...
    Share = colDef(name = "Share Sep 21", maxWidth = 80, align = "left"),
...
The wards with higher claimant rate in `r latestch` were `r arrange(table2, desc(latest_r))$area_name[1]` and `r arrange(table2, desc(latest_r))$area_name[2]` with `r arrange(table2, desc(latest_r))$latest_r[1]`% and `r arrange(table2, desc(latest_r))$latest_r[2]` respectably. The wards where the claimant rate has decreased more from `r latestm12ch` to `r latestch` are `r arrange(table2, Change)$area_name[1]` with a decrease in rate of `r arrange(table2, Change)$Change[1]`% followed by `r arrange(table2, Change)$area_name[2]`(`r arrange(table2, Change)$Change[2]`%), `r arrange(table2, Change)$area_name[3]`(`r arrange(table2, Change)$Change[1]`%) and `r arrange(table2, Change)$area_name[4]`(`r arrange(table2, Change)$Change[4]`%).
...
raw_cc_age <- read_csv("https://www.nomisweb.co.uk/api/v01/dataset/NM_162_1.data.csv?geography=1811939363&date=latestMINUS20-latest&gender=0&age=0,2,10,11,3,12...16,4,17...20&measure=1&measures=20100") 
...
query <- list(database = unbox("str:database:UC_Monthly"),
              ...
              recodes = list(
                ...
                `str:field:UC_Monthly:F_UC_DATE:DATE_NAME` = list(
                  map = as.list(paste0("str:value:UC_Monthly:F_UC_DATE:DATE_NAME:C_UC_DATE:",c(202001,202002,202003,202004,202005,202006,202007,202008,202009,202010,202011,202012,202101,202102,202103,202104,202105,202106,202107,202108,202109))))
              )) %>% toJSON()
request <- POST(
  url = path,
  body = query,
  ...
  geom_point(size = 5, alpha = 0.8, color="#66c2a4") + 
    geom_text(data=uc_plot_ch %>% filter(!period==base),
            aes(  
                label = paste0(round(change,0), "%")), 
            color = "#66c2a4", size = 3, hjust = 0, vjust = 2) +
  scale_x_date(breaks = uc_plot_ch$date, date_labels="%b %y") +
...
query <- list(database = unbox("str:database:UC_Monthly"),
              ...
              recodes = list(
                ...
                `str:field:UC_Monthly:F_UC_DATE:DATE_NAME` = list(
                  map = as.list(paste0("str:value:UC_Monthly:F_UC_DATE:DATE_NAME:C_UC_DATE:",c(202009,202109))))
              )) %>% toJSON()
request <- POST(
  url = path,
  body = query,
...
The Trafford MSOA with the largest share of claims in `r latestch` was `r filter(table7, Share == max(Share)) %>% pull(area_name)` with `r filter(table7, Share == max(Share))$Share`% of all Universal Credit claims in Trafford. The MSOA with the highest percentage change from `r latestm12ch` to `r latestch` were `r arrange(table7, desc(percentage_change))$area_name[1]`, `r arrange(table7, desc(percentage_change))$area_name[2]` and `r arrange(table7, desc(percentage_change))$area_name[3]` with `r arrange(table7, desc(percentage_change))$percentage_change[1]`% change. However, the MSOA with more additional claims in this period was `r arrange(table7, desc(additional_claims))$area_name[1]` and `r arrange(table7, desc(additional_claims))$area_name[2]` with `r arrange(table7, desc(additional_claims))$additional_claims[1]`, followed by `r arrange(table7, desc(additional_claims))$area_name[3]` (`r arrange(table7, desc(additional_claims))$additional_claims[3]`) and `r arrange(table7, desc(additional_claims))$area_name[4]` (`r arrange(table7, desc(additional_claims))$additional_claims[4]`).
...
query <- list(database = unbox("str:database:UC_Monthly"),
              ...
              recodes = list(
                `str:field:UC_Monthly:V_F_UC_CASELOAD_FULL:COA_CODE` = list(
                  map = as.list(paste0("str:value:UC_Monthly:V_F_UC_CASELOAD_FULL:COA_CODE:V_C_MASTERGEOG11_MSOA_TO_LA:E0", seq(2001259, 2001286, 1)))),
                `str:field:UC_Monthly:F_UC_DATE:DATE_NAME` = list(
                  map = as.list(paste0("str:value:UC_Monthly:F_UC_DATE:DATE_NAME:C_UC_DATE:",c(202109)))),
