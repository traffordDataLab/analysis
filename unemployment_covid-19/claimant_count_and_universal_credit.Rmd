...
cc_trafford <- read_csv("https://www.nomisweb.co.uk/api/v01/dataset/NM_162_1.data.csv?geography=1811939363&date=latestMINUS16-latest&gender=0&age=0&measure=1,2&measures=20100") %>%
...
The Trafford ward with the largest share of claims in `r latestch` was `r filter(c_count, Share == max(Share)) %>% pull(area_name)` with `r filter(c_count, Share == max(Share))$Share`% of all claimants within Trafford. Most of the wards have less claims in `r latestch`  compared to `r latestm12ch` when the claims peak during the first months of the Covid-19 pandemic. `r filter(c_count, additional_claims == min(additional_claims)) %>% pull(area_name)` has the largest reduction in number of claims with `r filter(c_count, additional_claims == min(additional_claims)) %>% pull(additional_claims)` followed by `r arrange(c_count, additional_claims)$area_name[2]` (`r arrange(c_count, additional_claims)$additional_claims[2]`), `r arrange(c_count, additional_claims)$area_name[3]` (`r arrange(c_count, additional_claims)$additional_claims[3]`), `r arrange(c_count, additional_claims)$area_name[4]` (`r arrange(c_count, additional_claims)$additional_claims[4]`) and `r arrange(c_count, additional_claims)$area_name[5]` (`r arrange(c_count, additional_claims)$additional_claims[5]`).
...
tbl2 <- reactable(
  ...
  columns = list(
    ...
    Share = colDef(name = "Share May 21", maxWidth = 80, align = "left"),
...
The wards with higher claimant rate in `r latestch` were `r filter(table2, latest_r == max(latest_r)) %>% pull(area_name)` with `r filter(table2, latest_r == max(latest_r))$latest_r`% and `r arrange(table2, desc(latest_r))$area_name[2]` with `r arrange(table2, desc(latest_r))$latest_r[2]`%. The wards where the claimant rate has decreased more from `r latestm12ch` to `r latestch` are `r filter(table2, Change == min(Change)) %>% pull(area_name)` and `r arrange(table2, Change)$area_name[2]` with a decrease in rate of `r arrange(table2, Change)$Change[1]`% and `r arrange(table2, Change)$Change[2]`% respectively along with `r arrange(table2, Change)$area_name[3]` with `r arrange(table2, Change)$Change[3]`%.
...
raw_cc_age <- read_csv("https://www.nomisweb.co.uk/api/v01/dataset/NM_162_1.data.csv?geography=1811939363&date=latestMINUS16-latest&gender=0&age=0,2,10,11,3,12...16,4,17...20&measure=1&measures=20100") 
...
query <- list(database = unbox("str:database:UC_Monthly"),
              ...
              recodes = list(
                ...
                `str:field:UC_Monthly:F_UC_DATE:DATE_NAME` = list(
                  map = as.list(paste0("str:value:UC_Monthly:F_UC_DATE:DATE_NAME:C_UC_DATE:",c(202001,202002,202003,202004,202005,202006,202007,202008,202009,202010,202011,202012,202101,202102,202103,202104,202105))))
              )) %>% toJSON()
request <- POST(
  url = path,
  body = query,
...
query <- list(database = unbox("str:database:UC_Monthly"),
              ...
              recodes = list(
                ...
                `str:field:UC_Monthly:F_UC_DATE:DATE_NAME` = list(
                  map = as.list(paste0("str:value:UC_Monthly:F_UC_DATE:DATE_NAME:C_UC_DATE:",c(202005,202105))))
              )) %>% toJSON()
request <- POST(
  url = path,
  body = query,
...
In Trafford from  `r latestm12ch` to `r latestch` the `r filter(table8, percentage_change == max(percentage_change)) %>% pull(Age)` age bands have the largest increment with `r filter(table8, percentage_change == max(percentage_change)) %>% pull(percentage_change)`% in the number of Universal Credit claims. When considering the number of additional claims the 5-year age band `r filter(table8, additional_claims == max(additional_claims)) %>% pull(Age)` years had the highest with `r filter(table8, additional_claims == max(additional_claims)) %>% pull(additional_claims)` more claims.
...
query <- list(database = unbox("str:database:UC_Monthly"),
              ...
              recodes = list(
                `str:field:UC_Monthly:V_F_UC_CASELOAD_FULL:COA_CODE` = list(
                  map = as.list(paste0("str:value:UC_Monthly:V_F_UC_CASELOAD_FULL:COA_CODE:V_C_MASTERGEOG11_MSOA_TO_LA:E0", seq(2001259, 2001286, 1)))),
                `str:field:UC_Monthly:F_UC_DATE:DATE_NAME` = list(
                  map = as.list(paste0("str:value:UC_Monthly:F_UC_DATE:DATE_NAME:C_UC_DATE:",c(202105)))),
