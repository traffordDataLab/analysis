...
cc_trafford <- read_csv("https://www.nomisweb.co.uk/api/v01/dataset/NM_162_1.data.csv?geography=1811939363&date=latestMINUS52-latest&gender=0&age=0&measure=1,2&measures=20100") %>%
...
tbl2 <- reactable(
  ...
  columns = list(
    ...
    Share = colDef(name = "Share May 24", maxWidth = 80, align = "left"),
...
The MSOAs with higher claimant rate in `r latestch` were `r arrange(table2, desc(latest_r))$area_name[1]` with `r arrange(table2, desc(latest_r))$latest_r[1]`%,  `r arrange(table2, desc(latest_r))$area_name[2]` with `r arrange(table2, desc(latest_r))$latest_r[2]`% and  `r arrange(table2, desc(latest_r))$area_name[3]` with `r arrange(table2, desc(latest_r))$latest_r[3]`%. From the top 5 MSOAs with the highest rates, only Partington had a lower rate when comparing `r latestm12ch` to `r latestch`. The MSOAs where the claimant rate have decreased more from `r latestm12ch` to `r latestch` were `r arrange(table2, Change)$area_name[1]` with `r arrange(table2, Change)$Change[1]`% points decrease, followed by  `r arrange(table2, Change)$area_name[2]` with `r arrange(table2, Change)$Change[2]`%.
...
raw_cc_age <- read_csv("https://www.nomisweb.co.uk/api/v01/dataset/NM_162_1.data.csv?geography=1811939363&date=latestMINUS52-latest&gender=0&age=0,2,10,11,3,12...16,4,17...20&measure=1&measures=20100") 
...
query <- list(database = unbox("str:database:UC_Monthly"),
              ...
              recodes = list(
                ...
                `str:field:UC_Monthly:F_UC_DATE:DATE_NAME` = list(
                  map = as.list(paste0("str:value:UC_Monthly:F_UC_DATE:DATE_NAME:C_UC_DATE:",c(202305,202405))))
              )) %>% toJSON()
request <- POST(
  url = path,
  body = query,
...
query <- list(database = unbox("str:database:UC_Monthly"),
              ...
              recodes = list(
                `str:field:UC_Monthly:V_F_UC_CASELOAD_FULL:COA_CODE` = list(
                  map = as.list(paste0("str:value:UC_Monthly:V_F_UC_CASELOAD_FULL:COA_CODE:V_C_MASTERGEOG11_MSOA_TO_LA:E0", seq(2001259, 2001286, 1)))),
                `str:field:UC_Monthly:F_UC_DATE:DATE_NAME` = list(
                  map = as.list(paste0("str:value:UC_Monthly:F_UC_DATE:DATE_NAME:C_UC_DATE:",c(202405)))),
...
As claimants are migrating from legacy benefits to Universal Credit and new claims are reported, the number of household receiving an entitlement have been increasing. From Feb 2023 to Feb 2024, both the proportion of household receiving Limited Capability for Work Entitlement and Child Entitlement have increased 1.8% points. The previous year, from Feb 2022 to Feb 2023, the Limited Capability for Work Entitlement proportion increased 3.7% points whilst the Child Entitlement proportion increased 1.5% points.
