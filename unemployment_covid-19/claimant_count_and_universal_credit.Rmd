...
cc_trafford <- read_csv("https://www.nomisweb.co.uk/api/v01/dataset/NM_162_1.data.csv?geography=1811939363&date=latestMINUS73-latest&gender=0&age=0&measure=1,2&measures=20100") %>%
...
latestch <- "February 2026"
latestm12ch <- "February 2025"
...
The Trafford MSOA with the largest share of claims in `r latestch` was `r filter(c_count, Share == max(Share)) %>% pull(area_name)` with `r filter(c_count, Share == max(Share))$Share`% of all claimants within Trafford, almost the double of claims than Gorse Hill, the MSOA with the second largest number of claims. 10 of the MSOA has more claims in `r latestch`  compared to `r latestm12ch`, and 2 MSOA had no additional claims. `r arrange(c_count, desc(additional_claims))$area_name[1]` have the largest increase in number of claims with `r abs(arrange(c_count, desc(additional_claims))$additional_claims[1])` more, followed by `r arrange(c_count, desc(additional_claims))$area_name[2]` and `r arrange(c_count, desc(additional_claims))$area_name[3]` with `r arrange(c_count, desc(additional_claims))$additional_claims[2]` additional claims each. `r arrange(c_count, additional_claims)$area_name[1]` had the most reduction in number of claims with `r abs(arrange(c_count, additional_claims)$additional_claims[1])` less followed by `r arrange(c_count, additional_claims)$area_name[2]` (`r abs(arrange(c_count, additional_claims)$additional_claims[2])`).
...
tbl2 <- reactable(
  ...
  columns = list(
    ...
    Share = colDef(name = "Share Feb 26", maxWidth = 80, align = "left"),
...
raw_cc_age <- read_csv("https://www.nomisweb.co.uk/api/v01/dataset/NM_162_1.data.csv?geography=1811939363&date=latestMINUS73-latest&gender=0&age=0,2,10,11,3,12...16,4,17...20&measure=1&measures=20100") 
...
In Trafford, from  `r latestm12ch` to `r latestch`, 7 out of 11 of the age bands has decreased the number of claimants. The age band `r filter(table3, additional_claims == max(additional_claims)) %>% pull(Age)`  had the most increase with `r filter(table3, additional_claims == max(additional_claims)) %>% pull(additional_claims)` more claimants. When considering the reduction of claims the age band `r filter(table3, additional_claims == min(additional_claims)) %>% pull(Age)` years had the largest reduction with `r filter(table3, additional_claims == min(additional_claims)) %>% pull(additional_claims)` less claims. Considering the broad age bands, only the age band 16-24 had increases.
...
uc <- as.data.frame.table(values, stringsAsFactors = FALSE) %>%
  ...
  filter(period %in% c(latestm12,latest)) %>% #c("Jun 2024", "Jun 2025")) %>%
  spread(period,value) %>%
  mutate(additional_claims= get(latest)-get(latestm12),
        percentage_change = round(additional_claims/get(latestm12) *100,0),
        `Share` = round(get(latest)/sum(get(latest)) *100,1)) %>%
