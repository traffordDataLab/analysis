...
cc_trafford <- read_csv("https://www.nomisweb.co.uk/api/v01/dataset/NM_162_1.data.csv?geography=1811939363&date=latestMINUS42-latest&gender=0&age=0&measure=1,2&measures=20100") %>%
...
tbl2 <- reactable(
  ...
  columns = list(
    ...
    Share = colDef(name = "Share Jul 23", maxWidth = 80, align = "left"),
...
The MSOAs with higher claimant rate in `r latestch` were `r arrange(table2, desc(latest_r))$area_name[1]` with `r arrange(table2, desc(latest_r))$latest_r[1]`%,  `r arrange(table2, desc(latest_r))$area_name[2]` with `r arrange(table2, desc(latest_r))$latest_r[2]`%,  `r arrange(table2, desc(latest_r))$area_name[3]` with `r arrange(table2, desc(latest_r))$latest_r[3]`% and `r arrange(table2, desc(latest_r))$area_name[4]` with `r arrange(table2, desc(latest_r))$latest_r[4]`%. From the top 4 MSOAs with the highest rates, Old Trafford and Firswood had a higher rate when comparing July 2022 to July 2023. The MSOAs where the claimant rate have decreased more from `r latestm12ch` to `r latestch` are `r arrange(table2, Change)$area_name[1]` with a decrease in rate of `r arrange(table2, Change)$Change[1]`%, `r arrange(table2, Change)$area_name[2]` with `r arrange(table2, Change)$Change[2]`% followed by `r arrange(table2, Change)$area_name[3]` with `r arrange(table2, Change)$Change[3]`%.
...
raw_cc_age <- read_csv("https://www.nomisweb.co.uk/api/v01/dataset/NM_162_1.data.csv?geography=1811939363&date=latestMINUS42-latest&gender=0&age=0,2,10,11,3,12...16,4,17...20&measure=1&measures=20100") 
...
query <- list(database = unbox("str:database:UC_Monthly"),
              ...
              recodes = list(
                ...
                `str:field:UC_Monthly:F_UC_DATE:DATE_NAME` = list(
                  map = as.list(paste0("str:value:UC_Monthly:F_UC_DATE:DATE_NAME:C_UC_DATE:",c(202207,202307))))
              )) %>% toJSON()
request <- POST(
  url = path,
  body = query,
...
query <- list(database = unbox("str:database:UC_Monthly"),
              ...
              recodes = list(
                `str:field:UC_Monthly:V_F_UC_CASELOAD_FULL:COA_CODE` = list(
                  map = list("str:value:UC_Monthly:V_F_UC_CASELOAD_FULL:COA_CODE:V_C_MASTERGEOG11_LA_TO_REGION:E08000009")),
                `str:field:UC_Monthly:F_UC_DATE:DATE_NAME` = list(
                  map = as.list(paste0("str:value:UC_Monthly:F_UC_DATE:DATE_NAME:C_UC_DATE:",c(202001,202002,202003,202004,202005,202006,202007,202008,202009,202010,202011,202012,202101,202102,202103,202104,202105,202106,202107,202108,202109,202110,202111,202112,202201,202202,202203,202204,202205,202206,202207,202208,202210,202211,202212,202301,202302,202303,202304,202305,202306,202307)))),
...
In Trafford from `r latestm12ch` to `r latestch` the `r filter(table8, percentage_change == max(percentage_change)) %>% pull(Age)` age band has the largest increment in percentage change of Universal Credit claims with `r filter(table8, percentage_change == max(percentage_change)) %>% pull(percentage_change)`%. When considering the number of additional claims from  `r latestm12ch` to `r latestch` the 5-year age band `r filter(table8, additional_claims == max(additional_claims)) %>% pull(Age)` years had the highest with `r filter(table8, additional_claims == max(additional_claims)) %>% pull(additional_claims)` more claims. The age bands `r arrange(table8, additional_claims)$Age[1]` and `r arrange(table8, additional_claims)$Age[2]` had a decrease in number of Universal Credit claims.
...
query <- list(database = unbox("str:database:UC_Monthly"),
              ...
              recodes = list(
                `str:field:UC_Monthly:V_F_UC_CASELOAD_FULL:COA_CODE` = list(
                  map = as.list(paste0("str:value:UC_Monthly:V_F_UC_CASELOAD_FULL:COA_CODE:V_C_MASTERGEOG11_MSOA_TO_LA:E0", seq(2001259, 2001286, 1)))),
                `str:field:UC_Monthly:F_UC_DATE:DATE_NAME` = list(
                  map = as.list(paste0("str:value:UC_Monthly:F_UC_DATE:DATE_NAME:C_UC_DATE:",c(202307)))),
