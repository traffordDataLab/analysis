...
cc_trafford <- read_csv("https://www.nomisweb.co.uk/api/v01/dataset/NM_162_1.data.csv?geography=1811939363&date=latestMINUS25-latest&gender=0&age=0&measure=1,2&measures=20100") %>%
...
latestch <- "February 2022"
latestm12ch <- "February 2021"
...
tbl2 <- reactable(
  ...
  columns = list(
    ...
    Share = colDef(name = "Share Feb 22", maxWidth = 80, align = "left"),
...
The wards with higher claimant rate in `r latestch` were `r arrange(table2, desc(latest_r))$area_name[1]` and `r arrange(table2, desc(latest_r))$area_name[2]` with `r arrange(table2, desc(latest_r))$latest_r[1]`% and `r arrange(table2, desc(latest_r))$latest_r[2]`% respectively. The wards where the claimant rate has decreased more from `r latestm12ch` to `r latestch` are `r arrange(table2, Change)$area_name[1]` and `r arrange(table2, Change)$area_name[2]` with a decrease in rate of `r arrange(table2, Change)$Change[1]`% and `r arrange(table2, Change)$Change[2]`% respectively followed by `r arrange(table2, Change)$area_name[3]` with `r arrange(table2, Change)$Change[3]`%.
...
raw_cc_age <- read_csv("https://www.nomisweb.co.uk/api/v01/dataset/NM_162_1.data.csv?geography=1811939363&date=latestMINUS25-latest&gender=0&age=0,2,10,11,3,12...16,4,17...20&measure=1&measures=20100") 
...
plotdf <- quinary %>%
 ...
 ggplot(plotdf, aes(x = date, y = value, colour = Age)) +
  ...
  scale_x_date(breaks = c(min(plotdf$date), as.Date(as.yearmon("Mar 2020"),format="%b %Y"), as.Date(as.yearmon("Jan 2021"),format="%b %Y"), max(plotdf$date)), date_labels = "%b %y") +
...
query <- list(database = unbox("str:database:UC_Monthly"),
              ...
              recodes = list(
                ...
                `str:field:UC_Monthly:F_UC_DATE:DATE_NAME` = list(
                  map = as.list(paste0("str:value:UC_Monthly:F_UC_DATE:DATE_NAME:C_UC_DATE:",c(202001,202002,202003,202004,202005,202006,202007,202008,202009,202010,202011,202012,202101,202102,202103,202104,202105,202106,202107,202108,202109,202110,202111,202112,202201,202202))))
              )) %>% toJSON()
request <- POST(
  url = path,
  body = query,
...
query <- list(database = unbox("str:database:UC_Monthly"),
              ...
              recodes = list(
                ...
                `str:field:UC_Monthly:F_UC_DATE:DATE_NAME` = list(
                  map = as.list(paste0("str:value:UC_Monthly:F_UC_DATE:DATE_NAME:C_UC_DATE:",c(202102,202202))))
              )) %>% toJSON()
request <- POST(
  url = path,
  body = query,
...
The Trafford MSOA with the largest share of claims in `r latestch` was `r filter(table7, Share == max(Share)) %>% pull(area_name)` with `r filter(table7, Share == max(Share))$Share`% of all Universal Credit claims in Trafford. All the MSOAs have less claims in `r latestch`  compared to `r latestm12ch`. The MSOA with more reduction in percentage change from `r latestm12ch` to `r latestch` was `r arrange(table7, percentage_change)$area_name[1]` with `r arrange(table7, percentage_change)$percentage_change[1]`% change. The MSOAs with the least reduction in claims from `r latestm12ch` to `r latestch` were `r arrange(table7, desc(additional_claims))$area_name[1]` with `r arrange(table7, desc(additional_claims))$additional_claims[1]`  and `r arrange(table7, desc(additional_claims))$area_name[2]` with `r arrange(table7, desc(additional_claims))$additional_claims[2]`. The MSOAs with more reduction in claims when comparing `r latestm12ch` to `r latestch` are `r arrange(table7,additional_claims)$area_name[1]` (`r arrange(table7,additional_claims)$additional_claims[1]`), `r arrange(table7,additional_claims)$area_name[2]` (`r arrange(table7, additional_claims)$additional_claims[2]`) and `r arrange(table7, additional_claims)$area_name[3]` (`r arrange(table7,additional_claims)$additional_claims[3]`).
...
uc_plot_age_c <- traf_uc %>%
 ...
 ggplot(uc_plot_age_c, aes(x = date, y = value, colour = Age)) +
  ...
  geom_text(aes(x = max(date), label = label, fontface = "bold", hjust = 0), show.legend = FALSE) +
  #scale_color_startrek() +
  scale_x_date(breaks = c(min(uc_plot_age_c$date), as.Date(as.yearmon("Mar 2020"),format="%b %Y"), as.Date(as.yearmon("Jan 2021"),format="%b %Y"), max(uc_plot_age_c$date)), date_labels = "%b %y") +
  scale_y_continuous(expand = c(0.005, 0.005)) +
...
uc_plot_age_r <- traf_uc %>%
 ...
 ggplot(uc_plot_age_r, aes(x = date, y = rate, colour = Age)) +
  ...
  geom_text(aes(x = max(date), label = label, fontface = "bold", hjust = 0), show.legend = FALSE) +
  #scale_color_startrek() +
  scale_x_date(breaks = c(min(uc_plot_age_r$date), as.Date(as.yearmon("Mar 2020"),format="%b %Y"), as.Date(as.yearmon("Jan 2021"),format="%b %Y"), max(uc_plot_age_r$date)), date_labels = "%b %y") +
  scale_y_continuous(expand = c(0.005, 0.005)) +
...
query <- list(database = unbox("str:database:UC_Monthly"),
              ...
              recodes = list(
                `str:field:UC_Monthly:V_F_UC_CASELOAD_FULL:COA_CODE` = list(
                  map = as.list(paste0("str:value:UC_Monthly:V_F_UC_CASELOAD_FULL:COA_CODE:V_C_MASTERGEOG11_MSOA_TO_LA:E0", seq(2001259, 2001286, 1)))),
                `str:field:UC_Monthly:F_UC_DATE:DATE_NAME` = list(
                  map = as.list(paste0("str:value:UC_Monthly:F_UC_DATE:DATE_NAME:C_UC_DATE:",c(202202)))),
