...
cc_trafford <- read_csv("https://www.nomisweb.co.uk/api/v01/dataset/NM_162_1.data.csv?geography=1811939363&date=latestMINUS17-latest&gender=0&age=0&measure=1,2&measures=20100") %>%
...
The rate of claimants to residents aged 16 to 64 reached `r filter(cc_traf,period==as.yearmon(latest, "%b %Y"))$rate`% in `r latestch` representing an increment of `r filter(cc_traf,period==as.yearmon(latest, "%b %Y"))$rate-cc_traf$rate[1]`% from `r basech`. From March 2021 the number of claimants are steadily reducing.
...
tbl2 <- reactable(
  ...
  columns = list(
    ...
    Share = colDef(name = "Share Jun 21", maxWidth = 80, align = "left"),
...
The wards with higher claimant rate in `r latestch` were `r arrange(table2, desc(latest_r))$area_name[1]` and `r arrange(table2, desc(latest_r))$area_name[2]` with `r arrange(table2, desc(latest_r))$latest_r[2]`%. The wards where the claimant rate has decreased more from `r latestm12ch` to `r latestch` are `r arrange(table2, Change)$area_name[1]` and `r arrange(table2, Change)$area_name[2]` with a decrease in rate of `r arrange(table2, Change)$Change[1]`% each along with `r arrange(table2, Change)$area_name[3]` with `r arrange(table2, Change)$Change[3]`%.
...
raw_cc_age <- read_csv("https://www.nomisweb.co.uk/api/v01/dataset/NM_162_1.data.csv?geography=1811939363&date=latestMINUS17-latest&gender=0&age=0,2,10,11,3,12...16,4,17...20&measure=1&measures=20100") 
...
query <- list(database = unbox("str:database:UC_Monthly"),
              ...
              recodes = list(
                ...
                `str:field:UC_Monthly:F_UC_DATE:DATE_NAME` = list(
                  map = as.list(paste0("str:value:UC_Monthly:F_UC_DATE:DATE_NAME:C_UC_DATE:",c(202001,202002,202003,202004,202005,202006,202007,202008,202009,202010,202011,202012,202101,202102,202103,202104,202105,202106))))
              )) %>% toJSON()
request <- POST(
  url = path,
  body = query,
...
query <- list(database = unbox("str:database:UC_Monthly"),
              ...
              recodes = list(
                ...
                `str:field:UC_Monthly:F_UC_DATE:DATE_NAME` = list(
                  map = as.list(paste0("str:value:UC_Monthly:F_UC_DATE:DATE_NAME:C_UC_DATE:",c(202006,202106))))
              )) %>% toJSON()
request <- POST(
  url = path,
  body = query,
...
The Trafford MSOA with the largest share of claims in `r latestch` was `r filter(table7, Share == max(Share)) %>% pull(area_name)` with `r filter(table7, Share == max(Share))$Share`% of all Universal Credit claims in Trafford. The MSOA with the highest percentage change from `r latestm12ch` to `r latestch` is `r filter(table7, percentage_change == max(percentage_change)) %>% pull(area_name)` with `r filter(table7, percentage_change == max(percentage_change)) %>% pull(percentage_change)`% change. However, the MSOA with more additional claims in this period is `r filter(table7, additional_claims == max(additional_claims)) %>% pull(area_name)` with `r filter(table7, additional_claims == max(additional_claims)) %>% pull(additional_claims)`, followed by `r arrange(table7, desc(additional_claims))$area_name[2]` (`r arrange(table7, desc(additional_claims))$additional_claims[2]`), `r arrange(table7, desc(additional_claims))$area_name[3]` (`r arrange(table7, desc(additional_claims))$additional_claims[3]`) and `r arrange(table7, desc(additional_claims))$area_name[4]` (`r arrange(table7, desc(additional_claims))$additional_claims[4]`).
...
query <- list(database = unbox("str:database:UC_Monthly"),
              ...
              recodes = list(
                `str:field:UC_Monthly:V_F_UC_CASELOAD_FULL:COA_CODE` = list(
                  map = as.list(paste0("str:value:UC_Monthly:V_F_UC_CASELOAD_FULL:COA_CODE:V_C_MASTERGEOG11_MSOA_TO_LA:E0", seq(2001259, 2001286, 1)))),
                `str:field:UC_Monthly:F_UC_DATE:DATE_NAME` = list(
                  map = as.list(paste0("str:value:UC_Monthly:F_UC_DATE:DATE_NAME:C_UC_DATE:",c(202106)))),
