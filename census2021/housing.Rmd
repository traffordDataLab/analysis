...
mv2_plot <- function(data, plotTitle, plotSubtitle, plotCaption, titleSize,subtitleSize, elemSize, stripSize){

gg<- ggplot(data, aes(Percent, category1)) +
  geom_col_interactive(aes(tooltip = tooltip), fill = "#902082") +
facet_grid(~category2, space = "free_y", scales = "free_y") +
    theme_lab() +
  labs(title = plotTitle,
       subtitle = plotSubtitle,
       caption = plotCaption,
       x = "% of Trafford households",
       y = "") +
    theme(plot.title = element_text(size=titleSize),
        ...
        strip.text.x = element_text(size = stripSize, face = "bold", hjust = 1, margin=margin(0,0,5,0))
        )
...
When comparing tenure by household composition in 2021 in the one-person household and single family household categories the larger proportion of households that owned outright were aged 66 years and over. The larger proportion of households that owned with a mortgage were married or civil partnership couples with dependent children. One-person households aged under 66 years had a similar proportion of all tenure categories. The largest proportion of lone parent family with dependent children social rented their accommodation. 
...
mv_tenure_hcomp_raw <- get_census_api_2021_mv2("https://www.nomisweb.co.uk/api/v01/dataset/NM_2235_1.data.csv?date=latest&geography=645922849&c2021_hhfamcomp_15=0...14&c2021_hhtenure_5=0...4&measures=20100")

mv_tenure_hcomp <- mv_tenure_hcomp_raw %>%
  filter(category1 != "Total", category2 != "Total") %>%
  ...
  mutate(category2 = case_when(category2 == "Private rented or lives rent free" ~ "**Private rented",
                          category2 == "Owned: Owns with a mortgage or loan or shared ownership" ~ "*Owns with a\nmortgage",
                          category2 == "Owned: Owns outright" ~ "Owns outright",
                          category2 == "Private rented or lives rent freePrivate rented or lives rent free" ~ "**Private rented",
                          TRUE ~category2
                          )) %>%
  mutate(category2 = factor(category2, levels = c("Owns outright", "*Owns with a\nmortgage", "**Private rented", "Social rented"))) %>%
  mutate(subcategory11 = factor(subcategory11, levels = c("One-person household", "Single family household", "Other household types"))) %>%
  mutate(subcategory12 = fct_rev(factor(subcategory12, levels = unique(subcategory12)))) %>%
  mutate(tooltip = paste0(Percent, "% (", format(value, big.mark=","),") of ", "<strong>", area_name,"</strong> households<br/>",
                          Percent2, "% of <strong>", category2,"</strong><br/>",
                          Percent1, "% of <strong>", subcategory11,":</strong><br/>",
                          "<strong>", subcategory12, "</strong>"
    ))
  

gg<- ggplot(mv_tenure_hcomp, aes(Percent, subcategory12)) +
  geom_col_interactive(aes(tooltip = tooltip), fill = "#5d77a3") +
facet_grid(subcategory11~category2, space = "free_y", scales = "free_y", switch = "y") +
    theme_lab() +
  labs(title = "Tenure by household composition",
       subtitle = "Trafford 2021",
       caption = "*Owns with a mortgage or loan or shared ownership. **Private rented or lives rent free.\nSource: Census 2021  |  @traffordDataLab",
       x = "% of Trafford households",
       y = "") +
    theme(plot.title = element_text(size=11, margin=margin(0,0,5,0)),
        ...
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        strip.text.x = element_text(size = 8, face = "bold", hjust = 1, margin=margin(0,0,5,0)),
        strip.placement = "outside",
        strip.text.y.left = element_text(size = 8, face = "bold", vjust = 1, hjust = 0, angle = 0),
...
mv_tenure_ethnic_raw <- get_census_api_2021_mv2("https://www.nomisweb.co.uk/api/v01/dataset/NM_2234_1.data.csv?date=latest&geography=645922849&c2021_eth_8=0...3,1001,7&c2021_hhtenure_5=0...4&measures=20100")

mv_tenure_ethnic <- mv_tenure_ethnic_raw %>%
  filter(category1 != "Total", category2 != "Total") %>%
  mutate(category1 = case_when(category1 == "Asian, Asian British or Asian Welsh" ~ "Asian, Asian British\nor Asian Welsh",
                               category1 == "Black, Black British, Black Welsh, Caribbean or African" ~ "Black, Black British,\nBlack Welsh, Caribbean or African",
                               TRUE ~ category1
                               )) %>%
  mutate(category2 = case_when(category2 == "Private rented or lives rent free" ~ "**Private rented",
                          category2 == "Owned: Owns with a mortgage or loan or shared ownership" ~ "*Owns with a\nmortgage",
  ...
  mutate(category2 = fct_rev(factor(category2, levels = c("Owns outright", "*Owns with a\nmortgage", "**Private rented", "Social rented")))) %>%
  mutate(category1 = fct_rev(factor(category1, levels = unique(category1)))) %>%
  mutate(tooltip = paste0(Percent, "% (", format(value, big.mark=","),") of ", "<strong>", area_name,"</strong> households<br/>",
                          Percent2, "% of <strong>", category2,"</strong><br/>",
                          Percent1, "% of <strong>", category1, "</strong>"
    ))

gg <- ggplot(mv_tenure_ethnic, aes(category1, Percent1, fill = category2, tooltip = tooltip)) + 
  geom_col_interactive(position = "stack", colour = "#757575", size = 0.2, alpha = 1) +
  scale_fill_manual(values = c("#48327F","#7987BA","#53478C","#9FBBD9"), #"#6665A2",
                     guide = guide_legend(keyheight = unit(3, units = "mm"),
  ...
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
    labs(title = "Tenure by ethnicity",
       subtitle = "Trafford 2021",
       caption = "*Owns with a mortgage or loan or shared ownership. **Private rented or lives rent free.\nSource: Census 2021  |  @traffordDataLab",
       x = NULL,
       y = "% of ethnic group",
...
girafe(ggobj = gg, height_svg = 4.5,
       options = girafe_options)
