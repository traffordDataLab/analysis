...
urlc <- "https://api.beta.ons.gov.uk/v1/population-types/UR/census-observations?area-type=ltla,E08000009&dimensions=ethnic_group_tb_6a,resident_age_3a,sex"

ethnicity_agec_sex_Trafford <- ons_api_3var(urlc) %>%
  filter(category1 != "Does not apply") %>%
  mutate(value = as.numeric(value))

ethnicity_age_sex_Trafford_pc <- ethnicity_agec_sex_Trafford %>% 
  ...
  mutate(Percent3 = round(value *100/sum(value),1)) %>%
  ungroup()

urlt <- "https://api.beta.ons.gov.uk/v1/population-types/UR/census-observations?area-type=ltla,E08000009&dimensions=ethnic_group_tb_6a,resident_age_3a"

ethnicity_aget_sex_Trafford <- ons_api_2var(urlt) %>%
  filter(category1 != "Does not apply") %>%
  mutate(value = as.numeric(value))

ethnicity_age_sex_Trafford_pt <- ethnicity_aget_sex_Trafford %>% 
  mutate(Percent = round(value *100/sum(value),2)) %>%
  group_by(category1) %>%
  mutate(Percent1 = round(value *100/sum(value),2)) %>%
  ungroup() %>%
...
ethnicity_age_sex_Trafford_2011 <- read_csv("https://www.nomisweb.co.uk/api/v01/dataset/NM_651_1.data.csv?date=latest&geography=1946157089&c_sex=1,2&c_age=1...21&c_ethpuk11=1,6,11,17,21&measures=20100") %>%
   select(date = DATE, area_name = GEOGRAPHY_NAME, category1 = C_ETHPUK11_NAME, category2 = C_AGE_NAME, category3 = C_SEX_NAME, measure = MEASURES_NAME, value = OBS_VALUE) %>%
  mutate(category1 = case_when(category1 == "White: Total" ~ "White",
                               category1 == "Mixed/multiple ethnic group: Total" ~ "Mixed or Multiple ethnic groups",
                               category1 == "Asian/Asian British: Total" ~ "Asian, Asian British or\nAsian Welsh",
                               category1 == "Black/African/Caribbean/Black British: Total" ~ "Black, Black British,\nBlack Welsh, Caribbean or African",
                               category1 == "Other ethnic group: Total" ~ "Other ethnic group",
                               TRUE ~ category1)) %>%
  mutate(category2b = gsub("Age ", "", category2)) %>%
  mutate(category3 = gsub("s", "", category3)) %>%
  mutate(category2b = gsub(" and over", "+", category2b)) %>%
  mutate(category2b = case_when( category2b == "5 to 7" |  category2b =="8 to 9" ~ "5 to 9",
                                 
                           category2b == "15" |  category2b =="16 to 17" |  category2b =="18 to 19" ~ "15 to 19",
                                 TRUE ~ category2b)) %>% 
  select(area_name,category1, category2b,category3, value) %>%
  group_by(area_name,category1, category2b,category3) %>%
  ...
  mutate(Percent2 = round(value *100/sum(value),2)) %>%
  ungroup() %>%
  group_by(category1,category2b) %>%
  mutate(Percent3 = round(value *100/sum(value),2)) %>%
  ungroup() %>%
  mutate(Percent12 = ifelse(category3 == "Male", -Percent1,Percent1)) %>%
...
ethnicity_age_sex_Trafford_p <- ethnicity_age_sex_Trafford %>% 
  ...
  left_join(ethnicity_age_sex_Trafford_2011 %>% select(area_name,category1,category2b,category3,value_2011 = value,Percent_2011 = Percent, Percent1_2011 = Percent1), by = c("area_name","category1","category2b","category3")) %>%
  mutate(tooltip = paste0(      "<strong>", category2,"</strong><br/>",
                                Percent1, "% of <strong>", category1,"</strong> (", Percent1_2011, "% in 2011)<br/>",
      "<strong>", category3,"</strong> ", Percent3, "%<br/>",
    "<strong>", "Residents: </strong>", format(value, big.mark=","), " (", Percent, "%) of Trafford<br/>",
    "<strong>", "2011: </strong>", format(value_2011, big.mark=","), " (", Percent_2011, "%) of Trafford<br/>"))

gg <- ggplot(ethnicity_age_sex_Trafford_p ) + 
  geom_col_interactive(aes(Percent12, category2b, fill = category3, tooltip = tooltip), alpha = 0.8) +
  geom_errorbar(data = ethnicity_age_sex_Trafford_2011,  aes(x=Percent12, xmax=Percent12, xmin=Percent12, y = category2b), color = "grey", size = 0.3) +
  facet_wrap(~category1,nrow = 3) + # scales = "free_x", 
  ...
  format(abs(x), ..., big.mark = ",", scientific = FALSE, trim = TRUE)
    ...
    labs(x = "% of residents of ethnic group", y = NULL, title = "Ethnicity by age and sex", subtitle = "Trafford 2021", caption = "Census 2011(line), Census 2021(bar)\nSource: ONS  |  @traffordDataLab",
  ...
  theme_lab() +
    theme(plot.title = element_text(size=12),
        ...
        strip.text.x = element_text(size = 7, face = "bold", hjust = 0, margin=margin(0,0,5,0))
        )
