...
library(tidyverse) ; library(ggplot2) ; library(ggspatial) ; library(ggthemes) ; library(ggrepel) ; library(sf) ; library(kableExtra) ; library(janitor) ; library(scales)
...
barcode_plot <- function (domain) {
  ...
  geom_segment(data = filter(df_iod_trafford, !lsoa11cd %in% lsoas_ward, index_domain == domain), 
               ...
               colour = "#18BC9E", size = 0.25, alpha = 1) +
  geom_segment(data = filter(df_iod_trafford, lsoa11cd %in% lsoas_ward, index_domain == domain), 
               aes(x = rank, xend = rank, y = 0, yend = 1), 
  ...
  geom_point(data = filter(df_iod_trafford, lsoa11cd %in% lsoas_ward, index_domain == domain),
               aes(x = rank, y = 1.03), pch = 25, colour = chart_highlight, fill = chart_highlight, size = 0.75) +
  geom_point(data = filter(df_iod_trafford, lsoa11cd %in% lsoas_ward, index_domain == domain),
               aes(x = rank, y = -0.03), pch = 24, colour = chart_highlight, fill = chart_highlight, size = 0.75) +
  geom_point(aes(x = c(0, 32844), y = -0.05), pch = 15, colour = "#18BC9E", size = 1) +
...
ggplot() +
  ...
  geom_sf(data = sf_lsoas_ward, fill = "#18BC9E", alpha = 1, colour = "#FFFFFF", size = 0.6) +
